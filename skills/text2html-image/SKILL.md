...
- Generated ImageGen assets may enter HTML `<img>` layers only after PNG format, real alpha transparency, dimensions, transparency provenance, and asset provenance are checked. Run `npm run audit:imagegen -- --input <candidates.json> --report <reports/imagegen-candidates.json>` on returned candidate files; RGB/no-alpha, matte, checkerboard, or missing `transparency_method` candidates must remain rejected with `blocked_from_final_html=true`. Valid candidate metadata must say whether alpha came from `native_alpha`, `chroma_key_removed`, or `flood_cutout`; chroma/flood outputs must keep the source path plus postprocess report/mask evidence.
...
- After ImageGen returns files, write or refresh `reports/imagegen-candidates.json` with `npm run audit:imagegen`. Do not promote a candidate to `split-art-assets.json`, `asset-provenance.json`, or HTML until the audit records alpha extrema, transparent corners, `transparency_method`, and `blocked_from_final_html=false`.
- Run `npm run audit:asset-readiness` before treating a case as asset-ready. Include `--routing <asset-routing-table.json>` when routing evidence exists so review-only route decisions and flattened-text photo conflicts are enforced. Asset-like expected routes must have final-ready provenance or explicit review-gate coverage; `prompt_only`, rejected ImageGen candidates, and planned cutouts are not usable final assets.
- For source-truth semantic assets such as QR/barcode, payment logos, app icons, and country flags, review-gated non-final assets need a source acquisition plan. Run `npm run audit:source-truth-acquisition` and forbid regenerated, approximate redraw, or editable-vector substitutes unless a real user-provided, licensed, source bitmap, or reference cutout asset is available.
- Review-gated source acquisition plans must include a machine-readable `blocking_condition` and `evidence_required` list so unresolved external source needs do not hide behind prose-only review notes.
- Final-ready source-truth assets must prove source type or route, source/output path, dimensions, checksum, and license/source scope evidence in `reports/asset-provenance.json`; `final_asset_ready=true` alone is not enough.
